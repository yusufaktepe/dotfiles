...
class fzf_locate(Command):
    ...
    def execute(self):
        import subprocess
        if self.quantifier:
            command="locate /home /data | fzf -e -i"
        else:
            command="locate /home /data | fzf -e -i"
...
class fzf_doc_search(Command):
    """
    :fzf_doc_search
    Search in PDFs, E-Books and Office documents in current directory.
    Allowed extensions: .epub, .odt, .docx, .fb2, .ipynb, .pdf.

    Usage: fzf_doc_search <search string>
    """
    def execute(self):
        if self.arg(1):
            search_string = self.rest(1)
        else:
            self.fm.notify("Usage: fzf_doc_search <search string>", bad=True)
            return
